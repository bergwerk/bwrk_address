...
var BwrkAddressMap = {
    ...
    measureDistance: function measureDistance(origin) {
        var lat = origin.lat();
        var lng = origin.lng();
        ...
        for (i = 0; i < this.markers.length; i++) {
            ...
            var a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(that.rad(lat)) * Math.cos(that.rad(lat)) * Math.sin(dLong / 2) * Math.sin(dLong / 2);
    ...
    setMapOnAll: function setMapOnAll(markers, map) {
    ...
    fitBounds: function fitBounds(markers) {
